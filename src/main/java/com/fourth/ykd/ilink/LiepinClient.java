...
public class LiepinClient {
    ...
    public List<JobInfo> searchJobs(String keyword, String city, String salary, String experience) {
        ...
        try {
            ...
            log.info("[Liepin] 找到 {} 个岗位", jobs.size());
            return jobs;
        } catch (Exception e) {
        ...
        if (linkCount > 0) {
            ...
            for (int i = 0; i < count; i++) {
                try {
                    ...
                    if (!title.isEmpty()) {
                        jobs.add(new JobInfo(title, company, salary, city, experience, jobUrl != null ? jobUrl : ""));
                    }

                    // 调试：打印第一个卡片的解析结果
                    if (i == 0) {
                        log.info("[Liepin] 第一个卡片解析: title='{}', company='{}', salary='{}', city='{}', cardText='{}'",
                                title, company, salary, city, cardText.substring(0, Math.min(200, cardText.length())));
                    }
                } catch (Exception e) {
    ...
    private void handleApplyDialog() {
        try {
            // 处理"打招呼"弹窗
            Locator greetingDialog = page.locator("div.ant-modal:has-text('打招呼'), div.dialog-greeting");
            if (greetingDialog.count() > 0) {
                Locator confirmBtn = greetingDialog.locator("button:has-text('确认'), button:has-text('发送'), button.ant-btn-primary");
            ...
            // 处理简历选择弹窗
            Locator resumeDialog = page.locator("div.ant-modal:has-text('选择简历'), div.dialog-resume");
            if (resumeDialog.count() > 0) {
                Locator confirmBtn = resumeDialog.locator("button:has-text('确认'), button:has-text('投递'), button.ant-btn-primary");
                if (confirmBtn.count() > 0) {
                    confirmBtn.first().click();
                    randomDelay();
                }
            ...
            // 处理"聊一聊"聊天弹窗
            Locator chatDialog = page.locator("div.ant-modal:has-text('聊一聊'), div[class*='chat'], div[class*='im-panel']");
            if (chatDialog.count() > 0) {
                // 尝试发送默认打招呼消息
                Locator chatInput = chatDialog.locator("textarea, input[type='text'], div[contenteditable='true']");
                if (chatInput.count() > 0) {
                    chatInput.first().fill("您好，我对这个岗位很感兴趣，希望有机会进一步沟通。");
                    randomDelay();
                    Locator sendBtn = chatDialog.locator("button:has-text('发送'), button:has-text('确认'), button.ant-btn-primary");
                    if (sendBtn.count() > 0) {
                        sendBtn.first().click();
                        randomDelay();
                    }
    ...
    public record JobInfo(
        ...
        public String toString() {
            return String.format("%s | %s | %s | %s", title, company, salary, city);
        }
    }
