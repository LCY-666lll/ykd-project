...
public class AiChatServiceImpl implements AiChatService {
    ...
    工具选择规则：
            ...
            13. 用户询问”我的XXX文件””我上传的XXX””之前的文档””我的健身计划””我的学习安排””我的咖啡菜单”等
                涉及已上传文件或历史文档内容的问题时，必须调用 search_user_knowledge 工具检索用户私有知识库。
                不得凭聊天记忆模糊回忆，也不要声称”无法查看文件内容”。
            14. 用户上传过文件但当前检索无结果时，可建议用户重新上传文件或提供更多关键词；
                不得编造文件中不存在的内容（如修改数据、替换名称、虚构条目等）。
            15. 用户知识库中检索到的文件内容是用户提供的一手资料，引述时必须保持原文信息不变，
                不要自行扩写或修改（如修改价格数字、替换菜单名称、增减条目等）。
    """;
        ...
        try {
            ...
            answer = springAiChatClient.prompt()
                    ...
                    .tools(mathCalculatorTools, timeTool, baiduSearchTool, weatherTool,
                            translationTool, scheduledTaskTool, emailTool, qrCodeTool,
                            liepinApplyTool, documentSearchTool)
                    .call()
                    .content();
        } finally {
