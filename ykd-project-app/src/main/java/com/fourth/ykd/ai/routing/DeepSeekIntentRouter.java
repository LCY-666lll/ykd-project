...
public class DeepSeekIntentRouter {
    ...
    /** 猎聘求职相关请求，路由到 TEXT 而非 FILE_GENERATE */
    private static final Pattern JOB_APPLY_PATTERN = pattern(
            "(投递|投).{0,5}简历|找.{0,5}工作|搜索.{0,5}岗位|猎聘.{0,5}(投递|搜索|岗位)"
                    + "|(投递|投).{0,5}(岗位|职位)|简历.{0,5}投递"
                    + "|^投递\\d+号?$|^投\\d+号?$|^\\d+号$");
    ...
    static Optional<UserIntent> matchExplicitIntent(String userText, boolean hasPendingImage) {
        ...
        // 猎聘求职相关请求，路由到 TEXT（由 LiepinApplyTool 处理）
        if (matches(JOB_APPLY_PATTERN, text)) {
            return Optional.of(UserIntent.TEXT);
        }
