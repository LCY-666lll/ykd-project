...
public class IlinkReplyProcessor {
    ...
    public ReplyResult process(String userId, String userText, boolean voiceMode) {
        // 精确短语"软删除记忆"：不走 AI 路由，直接清除会话记忆
        if (SoftDeleteChatMessage.TRIGGER_PHRASE.equals(userText.trim())) {
            softDeleteChatMessage.execute(userId);
            return ReplyResult.text(UserIntent.TEXT, SoftDeleteChatMessage.SUCCESS_REPLY, null);
        }
